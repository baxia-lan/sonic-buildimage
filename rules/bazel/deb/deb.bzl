...
def deb_package_set(
    ...
    _cmd = "\n".join([
        ...
        "  $$(docker image inspect sonic-builder:local >/dev/null 2>&1 && echo sonic-builder:local || echo " + _BUILD_IMAGE + ") \\",
        "  bash -o pipefail -c '",
        "    unset SOURCE_DATE_EPOCH",
        "    # Build deps pre-installed in @sonic_builder image (no apt-get needed)",
        "    # cppzmq headers from @cppzmq repo rule (fetched at bazel-fetch time)",
        "    if [ -f /usr/include/zmq.hpp ]; then true; else",
        "      echo 'WARN: zmq.hpp not in builder image, installing from network'",
        ...
        "    fi",
        "    if command -v cargo >/dev/null 2>&1; then true; else",
