...
def deb_package_set(
    ...
    _cmd = "\n".join([
        ...
        "    export MAKEFLAGS=-j2",
        "    PATH=/root/.cargo/bin:$$PATH DEB_BUILD_OPTIONS='nocheck parallel=2' dpkg-buildpackage -rfakeroot -b -us -uc -d -Pnoyangmod,nopython2 -j2 2>&1 | tail -100",
        "    echo \"=== debs produced ===\"",
        "    ls -lh /tmp/*.deb 2>/dev/null || echo \"NO DEBS\"",
        "    cp /tmp/*.deb /output/ 2>/dev/null || true",
        "    echo \"Files in /output/:\"",
        "    ls -lh /output/*.deb 2>/dev/null || echo \"NO DEBS IN /output/\"",
        "  '",
        "echo \"Files in OUT_DIR ($$OUT_DIR):\"",
        "ls -lh \"$$OUT_DIR\"/*.deb 2>/dev/null || echo \"NO DEBS IN OUT_DIR\"",
