...
def deb_package_set(
    ...
    _cmd = "\n".join([
        ...
        "# Temp dirs must live under the Bazel execroot so `docker run -v`",
        "# can see them from the host docker daemon. /tmp is not shared",
        "# across the DinD boundary on Cloud Build.",
