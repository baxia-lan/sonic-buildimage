...
case "${ARCH}" in
    ...
    arm64)
        ...
        ;;
    armhf)
        DOCKER_PLATFORM_ARG="--platform=linux/arm/v7"
        ;;
esac
...
docker create ${DOCKER_PLATFORM_ARG} --name $DOCKER_CONTAINER --entrypoint /bin/bash $DOCKER_IMAGE_TAG
...
DOCKER_BUILDKIT=1 docker build ${DOCKER_PLATFORM_ARG} -f ${DOCKER_PATH}/Dockerfile.cleanup  --target output -o target/vcache/${DOCKER_IMAGE_NAME} ${DOCKER_PATH}
DOCKER_BUILDKIT=1 docker build ${DOCKER_PLATFORM_ARG} -f ${DOCKER_PATH}/Dockerfile.cleanup  --no-cache --target final --tag ${DOCKER_IMAGE_TAG} ${DOCKER_PATH}
